...
const ShortCard = (props) => {
  ...
  const [subcribeStatus, setSubcribeStatus] = useState(null);
  ...
  const {
    ...
    currentTime,
      ...
      const fetchIsLikedOrNotShort = useCallback(
        async (shortIdActives) => {
          if (!shortIdActives) return "id not found";
          try {
            const likesChecking = await axiosInstance.get(
              `like/is-liked-or-not-short/${shortIdActives}`
            );
            setIsLiked(likesChecking?.data?.data);
          } catch (error) {
            console.error(error);
          }
        },
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      fetchIsLikedOrNotShort(short._id)
    },2000);

    return () => clearInterval(interval);
  },[short._id,fetchIsLikedOrNotShort])
  ...
  const fetchIsSubcribed = useCallback(async () => {
    if (!short.owner) return "id not found";
    try {
      const subcribe = await axiosInstance.get(
        `subcriber/is-subcribed/${short.owner}`
      );
      setSubcribeStatus(subcribe?.data?.data);
    } catch (error) {
      console.error(error);
    }
  }, [setSubcribeStatus, short.owner]);
  ...
  return (
    <div>
      <div
        ...
        onDoubleClick={() => fetchLikeToggle(short._id)}
        style={{
            ...
            <div className="w-full flex justify-between">
              ...
              <div className="flex items-center mr-2">
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    fetchSubcribeToggle(short.owner);
                  }}
                  className={`${
                    subcribeStatus === true
                ...
                >
                  {subcribeStatus === true ? "Subcribed" : "Subcribe"}
                </button>
              </div>
            </div>
              ...
              <div className="flex flex-col items-center">
                <button
                  onClick={(e) => {
                    ...
                    props.fetchShortComment(short?._id);
                  }}
